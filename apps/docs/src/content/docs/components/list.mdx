...
A low-chrome row — leading media, a title and description, trailing actions. Use `Item` when each row is a small record with an action or two. For tabular data with columns, use a [Table](../tables/); for a self-contained panel, use a [Card](../cards/).
