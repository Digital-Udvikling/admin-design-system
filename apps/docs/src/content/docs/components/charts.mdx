---
title: Charts
description: Pure-CSS bar, proportion, and donut primitives.
---

import { BarChart, Donut, StackedBar } from "@aortl/admin-react";
...
A chart is unreadable to a screen reader without a text summary. Each root needs `role="img"` and an `aria-label` restating the data — React generates one, vanilla does not.
...
`BarChart.Container` and the per-part components are the escape hatch for composing bars by hand — see the [`.Container` convention](../../basics/conventions/#container-escape-hatch).
