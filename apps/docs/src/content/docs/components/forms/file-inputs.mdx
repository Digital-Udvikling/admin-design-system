...
The size prop is `inputSize` because `<input>` has a native `size` attribute — see [Conventions › Sizes](../../../basics/conventions/#sizes). `type` is fixed to `"file"`. Plus native `<input>` attributes, including `accept`, `multiple` and `capture`.

There are no status variants beyond `danger`, and no `info` / `success` / `warning`, unlike [Inputs](../inputs/). For a label, description and validation, wrap it in a [Field](../fields/).
