...
`size="sm"` for confirms, `"md"` (default) for short forms, `"lg"` for longer ones, `"auto"` to shrink to content.
...
`size="metabase"` widens the modal to 1138px and the gutters to 44px, so a full-width body child — a Metabase iframe — lands at exactly 1048px.
