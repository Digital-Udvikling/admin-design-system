...
The anchor goes in `children`; the floating element is built from the props. With `label` or `icon` it is a [Badge](../badges/) and `size` applies; with neither it is a label-less dot and `size` is ignored. `max` clamps a numeric `label` to `${max}+`.
