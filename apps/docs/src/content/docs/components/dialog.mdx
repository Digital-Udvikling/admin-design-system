...
| `Dialog`             | `icon`         | [`IconProp`](../../basics/conventions/#icons)  | —         |
...
| `Dialog`             | `classNames`   | [slots](../../basics/conventions/#classnames)  | —         |
| `Dialog.Title`       | `icon`         | [`IconProp`](../../basics/conventions/#icons)  | —         |
| `Dialog.CloseButton` | `icon`         | [`IconProp`](../../basics/conventions/#icons)  | X glyph   |
...
`size` and `closedby` live on the element, so they work on [`Dialog.Container`](../../basics/conventions/#container-escape-hatch) too — the form to use when the layout doesn't fit, most often to wrap everything in a `<form>`. `Dialog.Title` keeps its `icon` prop there.
...
Write the close button's `aria-label` yourself. Don't put `overflow: hidden` on the root to clip the footer's fill — it also clips any [menu](../menus/) or [select](../forms/selects/) popup inside; the footer inherits the corner radius instead.

For an edge-anchored panel rather than a centred one, use a [Drawer](../drawer/).
