...
Pair `stat-card` with [`card`](../cards/) so it inherits every card surface and modifier. For free-form tiles, use [Cards](../cards/); for label/value tables, use [Property list](../property-list/).
...
`compact` tightens padding; `bordered` drops the shadow. Same [`card-compact` / `card-bordered`](../cards/#compact--bordered) modifiers as a card.
...
The same [variants as `<Card>`](../cards/#color-variants); the value picks up the matching accent. `warning` keeps its value at the default colour — yellow fails contrast on the tinted surface. Use sparingly.
...
A stat card has no compound parts, so `classNames` is the only way to reach the label, value, trend, and detail. See [Slot styling](../../basics/slots/).
