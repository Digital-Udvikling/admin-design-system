...
Portaled popups — [`<Select>`](../components/forms/selects.md) and [`<Tooltip>`](../components/tooltip.md) — render on `.popup-layer`, whose `z-index` reads `--z-popup` and defaults to `1000`. Lower it when an `<AdminRoot>` is embedded in a host page with chrome that should still paint above admin popups:
...
Declare it on `.admin-root` or any ancestor — the default is a `var()` fallback rather than a token, so any declaration wins outright.
