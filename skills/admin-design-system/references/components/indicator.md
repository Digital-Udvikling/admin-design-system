...
`<Indicator>` carries a single `content`. For two or more floating items on the same anchor, compose the CSS directly — each `.indicator-item` takes its own placement modifiers.
