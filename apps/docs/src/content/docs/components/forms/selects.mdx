...
The React `Select` is a Base UI compound primitive — `Select.Trigger`, `Select.Popup`, `Select.Item` — that renders a custom popup. The vanilla example uses a native `<select>` styled with `.select` so the class-name contract still holds, even though the popup chrome can only be customized in React.
