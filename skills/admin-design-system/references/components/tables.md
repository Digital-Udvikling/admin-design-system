...
**Caution** — There is no sort component — the click handler and ordering are yours. Set `aria-sort` to `ascending` or `descending` on the sorted column **only**; leaving it on every column tells a screen reader they are all sorted.
...
**Caution** — A table never owns a scroll region. `sticky`, `pinCol`, and horizontal overflow on narrow viewports all need you to supply the scrolling ancestor:
