...
A component is three or four files. Pick a kebab-case name (`badge`, `button-group`, `app-shell`) and reuse it as the base class.
...
Skip the class when the value is the default — the default look already lives in the base class. So `<Badge>`, `<Badge variant="neutral">`, and `<Badge size="md">` all render the same DOM as the bare `.badge` vanilla markup.
