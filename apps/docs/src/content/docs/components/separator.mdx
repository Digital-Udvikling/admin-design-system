...
A vertical rule needs `aria-orientation="vertical"` — the implicit `role="separator"` on `<hr>` reports horizontal otherwise. React's `orientation` prop sets it; in vanilla, write it yourself.
...
Margins are zeroed — spacing comes from the parent's `gap` or margin utilities. [Menus](../menus/) and [Breadcrumbs](../breadcrumbs/) keep their own scoped separators.
