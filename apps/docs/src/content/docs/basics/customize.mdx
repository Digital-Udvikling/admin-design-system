...
The fastest way to brand an app is the **system accent** — one CSS variable that drives the navbar stripe and [`<BrandTile>`](../../components/brand-tile/) at once. Everything else (palette tones, semantic roles, fonts) is still overridable; see [Advanced](#advanced).

## System accent

Set `--color-system-accent` at `:root` and your app picks up a 2px navbar stripe plus matching brand-tile color:
...
| -------------------------------- | ------------------------------- | ----------------------------------------- |
| `--color-system-accent`          | Navbar stripe, `.brand-tile` bg | The value you set                         |
