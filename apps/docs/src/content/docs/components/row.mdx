---
title: Row
description: One-dimensional layouts with flex utilities.
---

import { Badge, Button, Card, Input } from "@aortl/admin-react";
import { IconCircleCheck, IconFilter, IconPlus } from "@tabler/icons-react";
...
There is no `.row` class — see [Conventions › Layout](../../basics/conventions/#layout) for where the utilities come from. The class names are identical in both bundles, so the two tabs above differ only in `class` vs `className`.
...
For two-dimensional layouts see [Grid](../grid/); for spacing between stacked page sections, [Container](../container/). The full utility set is Tailwind's — these are the ones an admin layout reaches for.
