---
title: Vanilla CSS
description: One pre-built stylesheet, no build tooling required.
sidebar:
  order: 1
---
...
Embedding admin markup inside a non-admin page? See [Scoped bundle](../scoped/).
...
Class names are the API — see [Components](../../components/buttons/) for the catalog and [Conventions](../../basics/conventions/) for the sizes, tones, and layout rules that hold across all of them.
...
- **Colors** — semantic tokens only: `bg-primary`, `text-text-muted`, `border-danger`, `ring-info`, `shadow-success`. No raw Flexoki ramps (`bg-blue-600`) — see [Colors](../../basics/colors/).
...
Animations, transitions, filters, backdrop filters, gradient backgrounds, mix-blend, SVG fill/stroke, and transforms beyond `translate-x/y`. Dark mode comes from [tokens via `light-dark()`](../../basics/theming/#dark-mode), not a `dark:` variant. If a family you need is missing, the component CSS (`.btn`, `.card`, `.alert`, `.field`) usually covers it.
...
- [Icons](../../basics/icons/#install) — the Tabler webfont, one more `<link>`.
- [Theming](../../basics/theming/) — brand accent, dark mode, token overrides.
- [App shell](../../modules/app-shell/) — navbar, sidebar, and footer assembled into a page.
