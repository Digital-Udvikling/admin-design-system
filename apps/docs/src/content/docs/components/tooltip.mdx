...
The popup body accepts inline content such as a shortcut hint via [`<Kbd>`](../kbd/). `<Tooltip>` takes JSX in `content`; use subparts when `content` isn't enough.
...
The React popup is portaled onto `.popup-layer` — see [Customize › Popup layering](../../basics/customize/#popup-layering).
