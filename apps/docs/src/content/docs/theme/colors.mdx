...
import ColorCopy from "../../../components/ColorCopy.astro";

<ColorCopy />

The palette is [Flexoki](https://stephango.com/flexoki) — an inky, paper-warm scheme by Steph Ango. Colors live in two layers, both registered with Tailwind's `@theme`:
...
Use a utility (`bg-primary`, `text-text-muted`) or reference the variable directly (`var(--color-primary)`). Override either layer to reskin the system — see [Customize](../customize/).

Click any swatch to copy its hex, Tailwind class token, or CSS variable.

## Semantic tokens

Components only reference these — never the palette directly. Each family below shares the same four-slot layout: **Base**, **Hover** (interactive state), **Muted** (low-saturation background for alerts and tinted surfaces), and **Content** (text/icons on top of the base).
...
Every Flexoki tone is available as a utility class and CSS variable. Tones run light→dark from `50` to `950`; the base ramp is bookended by `paper` (warm white) and `black` (ink).
