...
An addon takes any content, including an icon — mark a decorative one `aria-hidden`. For a borderless glyph floating _inside_ the field instead, use [input icons](../inputs/#with-icons).
