...
Everything together. `<Container>` goes inside `<AppShell.Main>` (which has no padding of its own) and stacks the page sections — a stat grid, a [filter toolbar](../../components/tables/#filter-toolbar), and the [table](../../components/tables/). The toolbar's button opens a form [Drawer](../../components/drawer/); its `commandfor` matches the drawer's `id`.
