...
- **Voice is flat and factual.** No rhetorical color. Cut antithesis ("X rather than Y"), rule-of-three flavor triads, anthropomorphism ("digits don't shimmy"), marketing intensifiers ("the fastest way", "the simplest path"), empty reassurance ("text stays readable", "visible at a glance"), and soft hedges ("most likely"). Keep an em-dash only for a list or aside that adds information. Delete `###` intros that only narrate the next example.
- **Consumer pages carry the contract, not the plumbing.** Document the `_ao-` scope, class-name parity, version pinning, and props; leave the remark pipeline, virtual modules, and `@layer` ordering to the contributing pages.
