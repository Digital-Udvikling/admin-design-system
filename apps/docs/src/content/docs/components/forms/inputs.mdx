...
import { Input, PasswordInput } from "@aortl/admin-react";
...
| `Input`         | `icon`         | [`IconProp`](../../../basics/conventions/#icons)                        | —                 |
| `Input`         | `iconTrailing` | [`IconProp`](../../../basics/conventions/#icons)                        | —                 |
| `Input`         | `clearable`    | `boolean`                                                               | `false`           |
| `Input`         | `clearLabel`   | `string`                                                                | `"Clear"`         |
| `Input`         | `onClear`      | `() => void`                                                            | —                 |
| `Input`         | `action`       | `ReactNode`                                                             | —                 |
| `Input`         | `classNames`   | [slots](../../../basics/conventions/#classnames)                        | —                 |
| `PasswordInput` | `revealLabel`  | `string`                                                                | `"Show password"` |

The size prop is `inputSize` because `<input>` has a native `size` attribute — see [Conventions › Sizes](../../../basics/conventions/#sizes). `type` defaults to `"text"`.
...
Wraps [Base UI Input](https://base-ui.com/react/components/input), so inside a [Field](../fields/) it picks up the id, label association and validity wiring. Plus native `<input>` attributes.
...
For a bordered segment attached to the field instead of a floating glyph, use [input groups](../input-groups/). For the file picker, [FileInput](../file-inputs/); for steppers and clamping, [Number inputs](../number-inputs/).
