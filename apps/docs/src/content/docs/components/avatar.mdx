---
title: Avatar
description: Image with a no-JS initials fallback, plus a group stack.
---

import { Avatar, AvatarGroup, Indicator } from "@aortl/admin-react";
...
For an initials-only avatar sitting beside a visible name, mark it `aria-hidden` so the name isn't announced twice.
...
[Indicator](../indicator/) knows this component's sizes and shapes, so a presence dot lands on the right corner without an explicit offset.
