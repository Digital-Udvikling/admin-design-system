...
When a feature's behavior lives in the React component (clipboard access, focus traps, anything driven by `useState` + an effect / timer) and the vanilla bundle can't replicate it without consumer-written JS, drop the `html` fence so the example renders the React preview only. Mark the heading with Starlight's `<Badge>` so the divergence is visible at a glance:
...
```tsx
<PropertyList copyable>…</PropertyList>
```

:::
````

Alias the Starlight badge to `StarlightBadge` when the page already imports the admin `<Badge>` (a `Badge` collision is the common case, since most MDX pages bring in admin components by name). Keep the supporting CSS classes shipping in both bundles — consumers wiring their own vanilla JS still need the styling.
