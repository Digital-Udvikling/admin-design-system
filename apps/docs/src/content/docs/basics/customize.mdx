...
Portaled popups — [`<Select>`](../../components/forms/selects/) and [`<Tooltip>`](../../components/tooltip/) — render on `.popup-layer`, whose `z-index` reads `--z-popup` and defaults to `1000`. Lower it when an `<AdminRoot>` is embedded in a host page with chrome that should still paint above admin popups:
