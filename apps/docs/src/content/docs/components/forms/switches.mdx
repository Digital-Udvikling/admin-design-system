...
A wrapping `<label>` is laid out for you: inline row, `0.75rem` gap (wider than a checkbox's, since the track is wider), pointer cursor, dimmed when disabled. Pair it with [`field-row`](../fields/#inline-label) for a label on the same line as the control.

A switch applies its change immediately. For something that only takes effect on submit, use a [checkbox](../checkboxes/).
