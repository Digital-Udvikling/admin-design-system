...
A `<dl>` with a label column that aligns across every row. Use it for the "one entity, N attributes" pattern: summary panels, identifier strips, metadata blocks. See [Table](../tables/) for tabular/multi-row data.
