---
title: List
description: Compact rows for settings, members, and notifications.
---

import { Item, ItemGroup, Avatar, Badge, Button } from "@aortl/admin-react";
...
| `Item`      | `icon`        | [`IconProp`](../../basics/conventions/#icons) | —           |
| `Item`      | `title`       | `ReactNode`                                   | —           |
| `Item`      | `description` | `ReactNode`                                   | —           |
| `Item`      | `actions`     | `ReactNode`                                   | —           |
| `Item`      | `classNames`  | [slots](../../basics/conventions/#classnames) | —           |
...
`variant`, `size` and `asLink` live on the row, so they work on both `Item` and [`Item.Container`](../../basics/conventions/#container-escape-hatch). `asLink` styles the row and expands the first nested link, but you supply the `<a>`: put it in the `title`. `ItemGroup` is a standalone export, not `Item.Group`.
...
For tabular data with columns use a [Table](../tables/); for a self-contained panel, a [Card](../cards/).
