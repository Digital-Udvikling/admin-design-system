...
Put a [Checkbox](/admin-design-system/components/forms/checkboxes/) in the first cell — row tint is driven by `:has(input:checked)`, no state mirror needed. Wire the select-all header checkbox yourself.
