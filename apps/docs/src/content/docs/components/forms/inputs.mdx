...
import { Field, Input, PasswordInput } from "@aortl/admin-react";
...
The `.input-icon` wrapper floats a borderless muted icon inside the field. Position is structural — an `i`/`svg` before the `.input` is leading, after is trailing. For a bordered segment instead, use [input groups](../input-groups/). See [Icons](../../../basics/icons/).
